...
echo "$OUT" | jq '{relay_uri:.payload.relay_uri,track_name:.payload.track_name,chunks_published:.payload.chunks_published,first_chunk_latency_ms:.payload.first_chunk_latency_ms,subscriber_ready_at_pct:.payload.subscriber_ready_at_pct}'
RELAY=$(echo "$OUT" | jq -r '.payload.relay_uri')
CHUNKS=$(echo "$OUT" | jq -r '.payload.chunks_published')
READY=$(echo "$OUT" | jq -r '.payload.subscriber_ready_at_pct')
...
echo "$OUT" | jq '{relay_uri:.payload.relay_uri,track_name:.payload.track_name,config:.payload.config}'
RELAY=$(echo "$OUT" | jq -r '.payload.relay_uri')
TRACK=$(echo "$OUT" | jq -r '.payload.track_name')
CMS=$(echo "$OUT" | jq -r '.payload.config.chunk_ms')
...
echo "$OUT" | jq '{dry_run:.payload.dry_run,chunks_published:.payload.chunks_published,bytes_streamed:.payload.bytes_streamed}'
DR=$(echo "$OUT" | jq -r '.payload.dry_run')
CP=$(echo "$OUT" | jq -r '.payload.chunks_published')
BS=$(echo "$OUT" | jq -r '.payload.bytes_streamed')
...
PROOF_COUNT=$(echo "$OUT" | jq '.payload.proof_per_chunk | length')
FIRST_PROOF=$(echo "$OUT" | jq -r '.payload.proof_per_chunk[0].proof_hash')
echo "$OUT" | jq '.payload.proof_per_chunk'
...
OUT=$($AKAI_CMD weights moq-stream --model mistral-7b.q4.akmodel 2>/dev/null)
echo "$OUT" | jq '.payload.stream_plan'
P1=$(echo "$OUT" | jq -r '.payload.stream_plan.phase_1_hot_tensors')
P2=$(echo "$OUT" | jq -r '.payload.stream_plan.phase_2_routing')
P3=$(echo "$OUT" | jq -r '.payload.stream_plan.phase_3_remaining')
...
echo "$OUT" | jq '{schema_version:.schema_version,command:.command,relay_uri:.payload.relay_uri,dry_run:.payload.dry_run}'
